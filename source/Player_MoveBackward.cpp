...
Player_MoveBackward::Player_MoveBackward (const IDType & playerID)
{
	_playerID = playerID;
}

void Player_MoveBackward::getMoves(GameState & state, const MoveArray & moves, std::vector<Action> & moveVec)
{
    moveVec.clear();
    ...
    {
        avgPos.addPosition(state.getUnit(enemy, u).pos());
    }

    avgPos.scalePosition(1.0f / state.numUnits(enemy));
