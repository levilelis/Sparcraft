...
AlphaBetaValue AlphaBetaSearch::IDAlphaBeta(GameState & initialState, const size_t & maxDepth)
{
	...
	for (size_t d(1); d < maxDepth; ++d)
	{
		...
		catch (int e)
		{
			...
			if (d == 1)
			{
				printf("Selecting best scripted move for AB");
				MoveArray moves;
...
bool AlphaBetaSearch::getNextMoveVecAllOrdered(IDType playerToMove, MoveArray & orderedMoves, MoveArray & moves, const size_t & moveNumber, const TTLookupValue & TTval, const size_t & depth, std::vector<Action> & moveVec) const
{
	...
	if (orderedMoves.hasMoreMoves())
	{
		orderedMoves.getNextMoveVec(moveVec);
		return true;
	}
	else
	{
...
//	MoveArray orderedMoves;
//	generateOrderedMovesOnly(state, orderedMoves, playerToMove, depth);
 ...
 //   while (getNextMoveVecAllOrdered(playerToMove, orderedMoves, moves, moveNumber, TTval, depth, moveVec))
	{
    	//printf("Move number %lu, and param %lu, for player %d\n", moveNumber, _params.maxChildren(), playerToMove);
  ...
  //  printf("Number of moves considered was %lu by player %d \n", moveNumber, playerToMove);
...
void AlphaBetaSearch::generateOrderedMovesOnly(GameState & state, MoveArray & moves, const IDType & playerToMove, const size_t & depth)
{
	moves.clear();
	Array<std::vector<Action>, Constants::Max_Ordered_Moves> & orderedMoves(_orderedMoves[depth]);
	std::map<IDType, std::vector<Action> > unit_actions;

	for(int i = 0; i < orderedMoves.size(); i++)
	{
		for(int j = 0; j < orderedMoves[i].size(); j++)
		{
			if(unit_actions.find(orderedMoves[i][j].unit()) == unit_actions.end())
			{
				std::vector<Action> vec;
				unit_actions[orderedMoves[i][j].unit()] = vec;
			}
			unit_actions[orderedMoves[i][j].unit()].push_back(orderedMoves[i][j]);
		}
	}

	for(std::map<IDType, std::vector<Action> >::iterator it = unit_actions.begin(); it != unit_actions.end(); ++it)
	{
		moves.addUnit();

		for(int i = 0; i < it->second.size(); i++)
		{
			moves.add((it->second)[i]);
		}
	}
}
